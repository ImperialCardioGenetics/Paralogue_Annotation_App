...
fluidPage(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      The original methodology and implementation of [Paralogue Annotation on arrhythmia syndrome genes](https://www.cardiodb.org/paralogue_annotation/) was published [here](https://onlinelibrary.wiley.com/doi/full/10.1002/humu.22114) and [here](https://jmg.bmj.com/content/51/1/35). 
      This web app extends Paralogue Annotation exome-wide, using paralogues defined by [Ensembl gene trees](https://www.ensembl.org/Help/View?id=137) and pathogenic/likely pathogenic missense variants defined by [ClinVar](https://www.ncbi.nlm.nih.gov/clinvar/).</p>
      <br>
      <p>This web app is currently being built using [Shiny](http://shiny.rstudio.com), the source code is available at [https://github.com/ImperialCardioGenetics/Paralogue_Annotation_App](https://github.com/ImperialCardioGenetics/Paralogue_Annotation_App).</p>
      <br>
      <br>
      <br>
      ...
      <b>A.</b> Currently only GRCh37 coordinates are supported. We recommend using [Ensembl liftover service](https://www.ensembl.org/Homo_sapiens/Tools/AssemblyConverter) for coordinate conversions.
      <br>
      <br>
      <b>Q. What are the Para_z scores?</b>
      <br>
      <b>A.</b> The Para_z scores are a measure of paralogue conservation independently derived by [_Lal et al._ (2020)](https://genomemedicine.biomedcentral.com/articles/10.1186/s13073-020-00725-6). You may therefore find in your results that some Para_z scores do not agree with your expectations. 
      This is because the paralogue alignments used to generate the scores are different to the alignments used here. The Para_z scores can thus be thought as a third-party confidence score of paralogue conservation across aligned positions.
      <br>
      <br>
      ...
      <br>
      <b>A.</b> This is mainly because the original Paralogue Annotation utilized T-COFFEE for the alignments, whereas Ensembl alignments are generated by CLUSTAL W instead. Furthermore, variants from [HGMD](http://www.hgmd.cf.ac.uk/ac/index.php) were used instead of [ClinVar](https://www.ncbi.nlm.nih.gov/clinvar/).
      <br>
      <br>
      <b>Q. What formats do my input variants have to be in?</b>
      <br>
      <b>A.</b> Currently variants have to be submitted using their chromosome, position, reference allele, and alternate allele using any delimiter in the format of 'CHROM:POS:REF:ALT' with separate variants on newlines. 
      Alternatively we also accept [VCF](https://www.internationalgenome.org/wiki/Analysis/vcf4.0/) as well.
      <br>
      <br>
